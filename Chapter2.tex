...
%To insert alt-text to images in Word, hover over the image > right click > view alt text > input the description for the image
...
If the first column contains row headers, use \verb|\tagpdfsetup{table/header-columns={1}}|. For a table such as a distance matrix that has column headers in the first row and row headers in the first column (e.g., \autoref{tab:city_distance}), combine both settings as \verb|\tagpdfsetup{table/header-rows={1},table/header-columns={1}}|. See the \LaTeX{} code example in  \autoref{tab:city_distance}.

% compatibility note: If using tabularx package, p and X column types are not compatible with header-columns in TeX 2025. This support will likely be added in future TeX releases. Use other types of columns such as l type if you want to define a header column
% other non-header columns in the table can be p or X
\tagpdfsetup{
    table/header-rows={1},
...
\textbf{Point A} & --- &     &     &     &     \\ \hline
\textbf{Point B} & 87  & --- &     &     &     \\ \hline
\textbf{Point C} & 64  & 56  & --- &     &     \\ \hline
\textbf{Point D} & 37  & 32  & 91  & --- &     \\ \hline
\textbf{Point E} & 93  & 35  & 54  & 43  & --- \\ \hline
\end{tabularx}
\end{table}
...
\TeX{} Live 2025 with the LuaLaTeX compiler auto tags math equations as “Formula” and attaches the corresponding MathML to the PDF. Compatible PDF viewers, such as Adobe Acrobat, can use this MathML to make equations available to assistive technologies such as screen readers. Associated MathML files can appear in Acrobat’s Attachments panel.
$$ \int_1^{12} \frac{x^2}{5 - \sqrt{5}} \,dx $$

Equations can also be numbered with a label.
